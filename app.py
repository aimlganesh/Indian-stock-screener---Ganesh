...
# Expert Balanced Scoring Weights (Total = 100%)
WEIGHTS = {
    'Value_Score': 0.25,
    'Momentum_Score': 0.25,
    'Quality_Score': 0.25,
...
def get_eps_cagr(ticker, periods=[1, 3, 5]):
    """Calculates EPS CAGR over multiple periods (mocked using Beta)."""
    try:
...
def get_free_cash_flow(info):
    """Pulls FCF from yfinance if available, otherwise returns None (using OCF proxy)."""
    try:
        # Use Operating Cash Flow as a proxy for FCF availability
...
def check_stable_earnings(ticker):
    """Checks for stable earnings (mocked)."""
...
def z_score_normalize(df, metric_col, inverse=False, target_range=10):
    """
    Applies Z-Score normalization and scales to a 0-10 score.
...
def get_category_recommendation(score):
    """V33 Enhancement: Converts a 0-10 normalized score to a rating (HTML span for display)."""
    if score is None:
...
def calculate_technical_score(signals):
    ...
    # 3. MACD Crossover (3 points) - Proxy using RSI
    if rsi > 55: 
        score += 3
...
def plot_candlestick(data, view_mode):
    ...
    fig.add_hline(y=40, line_dash="dash", line_color="orange", row=2, col=1) 
